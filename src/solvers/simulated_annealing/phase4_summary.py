"""
Build Phase 4 ASA summary CSV from the per-seed detail CSV.
"""
...
DETAIL_PATH = os.path.join(PHASE4_ROOT, "asa_detail.csv")
OUTPUT_PATH = os.path.join(PHASE4_ROOT, "asa.csv")
FIELDNAMES = [
    "testcase",
    "cost_min",
    "cost_max",
...
def main() -> None:
    ...
    with open(DETAIL_PATH, encoding="utf-8", newline="") as stream:
        ...
        if "t_best" not in fieldnames:
            raise RuntimeError(
                f"{DETAIL_PATH} is missing t_best. Re-run src/solvers/simulated_annealing/phase4.py first."
            )
        for row in reader:
