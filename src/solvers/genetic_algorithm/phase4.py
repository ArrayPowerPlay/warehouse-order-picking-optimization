"""
Phase 4 detail runner for Genetic Algorithm.

This script scans all test_set testcases, selects the best Phase 3 GA
...
DEFAULT_OUTPUT_PATH = os.path.join(PHASE4_ROOT, "ga_detail.csv")
...
def load_best_configs() -> dict[str, tuple[int, float, float]]:
    """Load best GA configuration per size group from Phase 3 output."""
    if not os.path.isfile(PHASE3_CONFIG_PATH):
        raise FileNotFoundError(f"Missing Phase 3 GA config file: {PHASE3_CONFIG_PATH}")
    ...
    with open(PHASE3_CONFIG_PATH, encoding="utf-8", newline="") as stream:
        ...
        for row in reader:
            best_configs[row["group"]] = (
                int(row["pop_size"]),
                float(row["crossover_rate"]),
                float(row["mutation_rate"]),
            )

    if not best_configs:
        raise RuntimeError(f"No Phase 3 GA configurations found in {PHASE3_CONFIG_PATH}")
    return best_configs
...
def run_single_seed(
    ...
    """Run one GA configuration on one seed and return total_distance and t_best."""
    with open(input_path, encoding="utf-8") as stream:
        original_stdin = sys.stdin
        try:
            sys.stdin = stream
            _, total_distance, t_best = ga_solver(
...
def build_phase4_tasks(
    ...
    for testcase_name, input_path, size_bucket, time_limit in discover_testcases(selected_testcases):
        if size_bucket not in best_configs:
            print(f"[WARNING] Missing GA config for group '{size_bucket}', skip testcase {testcase_name}")
            continue
...
def run_phase4_task(task: tuple[str, str, float, int, float, float, int]) -> dict[str, object]:
    ...
    if effective_workers <= 1:
        for task in tasks:
            ...
            print(
                ...
                f"mutation_rate={row['mutation_rate']} | k={row['k']} | "
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Phase 4 detail runner for Genetic Algorithm")
    parser.add_argument(
        "--testcase",
        action="append",
