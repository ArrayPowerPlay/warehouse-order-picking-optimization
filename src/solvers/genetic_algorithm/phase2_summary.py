"""
Build Phase 2 GA summary CSV from the per-seed detail CSV.
"""
...
DETAIL_PATH = os.path.join(PHASE2_ROOT, "ga_detail.csv")
OUTPUT_PATH = os.path.join(PHASE2_ROOT, "ga.csv")
FIELDNAMES = [
    "testcase",
    "pop_size",
    "crossover_rate",
...
def sort_key(row: dict[str, object]) -> tuple[str, int, float, float]:
...
def main() -> None:
    if not os.path.isfile(DETAIL_PATH):
        raise FileNotFoundError(f"Missing GA detail file: {DETAIL_PATH}")

    grouped_costs: dict[tuple[str, int, float, float], list[int]] = {}
    ...
    rows = []
    for (testcase, pop_size, crossover_rate, mutation_rate), costs in grouped_costs.items():
