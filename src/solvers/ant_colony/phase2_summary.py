"""
Build Phase 2 ACO summary CSV from the per-seed detail CSV.
"""
...
DETAIL_PATH = os.path.join(PHASE2_ROOT, "aco_detail.csv")
OUTPUT_PATH = os.path.join(PHASE2_ROOT, "aco.csv")
FIELDNAMES = [
    "testcase",
    "num_ants",
    "alpha",
...
def sort_key(row: dict[str, object]) -> tuple[str, int, float, float, float]:
...
def main() -> None:
    ...
    grouped_costs: dict[tuple[str, int, float, float, float], list[int]] = {}
    ...
    rows = []
    for (testcase, num_ants, alpha, beta, rho), costs in grouped_costs.items():
