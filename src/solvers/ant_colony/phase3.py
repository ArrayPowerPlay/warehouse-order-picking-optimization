...
def compute_small_avg_rfd(
    ...
    for config_idx, (num_ants, alpha, beta, rho) in enumerate(hyperparameter_grid, start=1):
        ...
        avg_rfd_by_key[(num_ants, alpha, beta, rho, "small")] = sum(rfd_values) / len(rfd_values)
...
def select_best_config_per_group(
    avg_rfd_by_key: dict[tuple[int, float, float, float, str], float]
) -> list[dict[str, object]]:
    """Select the minimum avg_RFD configuration for each available group."""
    best_rows_by_group: dict[str, dict[str, object]] = {}

    for (num_ants, alpha, beta, rho, group), avg_rfd in avg_rfd_by_key.items():
        candidate = {
            "num_ants": num_ants,
            "alpha": alpha,
            "beta": beta,
        ...
        candidate_key = (avg_rfd, num_ants, alpha, beta, rho)
        current_key = (
